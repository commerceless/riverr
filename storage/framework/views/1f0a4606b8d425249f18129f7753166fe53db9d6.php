        ...
        <table class="w-full whitespace-nowrap old-tables">
            <thead class="bg-gray-200">
                <tr tabindex="0" class="focus:outline-none h-16 w-full text-sm leading-none text-gray-800 dark:text-white ">
                    ...
                    <th class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider ltr:text-left ltr:pl-4 rtl:text-right rtl:pr-4"><?php echo e(__('messages.t_payment')); ?></th>
                    <th class="font-bold text-[10px] text-slate-500 dark:text-gray-300 uppercase tracking-wider text-center"><?php echo e(__('messages.t_total_amount')); ?></th>
            ...
            <tbody class="w-full">
                ...
                <?php $__currentLoopData = $invoices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invoice): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($invoice->order->buyer): ?>
                        <tr class="focus:outline-none h-20 text-sm leading-none text-gray-800 bg-white dark:bg-zinc-600 hover:bg-gray-100 dark:hover:bg-zinc-700 border-b border-t border-gray-100 dark:border-zinc-700/40" wire:key="invoices-<?php echo e($invoice->id); ?>">

                            
                            <td class="ltr:pl-4 rtl:pr-4">
                                <div class="flex items-center">
                                    <div class="w-8 h-8">
                                        <img class="w-full h-full rounded object-cover lazy" src="<?php echo e(placeholder_img()); ?>" data-src="<?php echo e(src($invoice->order->buyer->avatar)); ?>" alt="<?php echo e($invoice->order->buyer->username); ?>" />
                                    </div>
                                    <div class="ltr:pl-4 rtl:pr-4">
                                        <a href="<?php echo e(admin_url('users/details/' . $invoice->order->buyer->uid)); ?>" target="_blank" class="font-medium text-xs hover:text-primary-600 truncate pb-1.5 block max-w-xs"><?php echo e($invoice->order->buyer->username); ?></a>
                            ...
                            <td class="text-center">
                                <span class="text-xs font-black "><?php echo money($invoice->order->total_value, settings('currency')->code, true); ?></span>
                            </td>
                            ...
                            <td class="text-center">
                                <div class="relative inline-block text-left">
                                    ...
                                    <div id="table-options-dropdown-<?php echo e($invoice->id); ?>" class="hidden z-40 origin-top-right absolute right-0 mt-2 w-44 rounded-md shadow-lg bg-white dark:bg-zinc-800 ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-zinc-700 focus:outline-none" role="menu"  aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                        <div class="py-1" role="none">

                                            
                                            <a href="<?php echo e(admin_url('orders/details/' . $invoice->order->uid)); ?>" target="_blank" class="text-gray-800 dark:text-gray-300 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                                            ...
                                            <?php if($invoice->status === 'pending'): ?>
                                                <button x-on:click="confirm('<?php echo e(__('messages.t_are_u_sure_u_received_invocie_payment')); ?>') ? $wire.paid('<?php echo e($invoice->id); ?>') : ''" wire:loading.attr="disabled" wire:target="paid('<?php echo e($invoice->id); ?>')" type="button" class="text-gray-800 dark:text-gray-300 dark:hover:text-gray-400 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" >
...
<?php /**PATH /var/www/html/riverr/resources/views/livewire/admin/invoices/invoices.blade.php ENDPATH**/ ?>