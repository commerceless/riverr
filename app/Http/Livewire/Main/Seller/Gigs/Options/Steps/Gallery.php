...
class Gallery extends Component
{
    ...
    public $video_link;
    public $video_id;
    public $vimeo_video_link;
    public $vimeo_video_id;
    public $thumbnail;
    ...
    public function mount(Gig $gig)
    {
        ...
        $this->fill([
            ...
            'vimeo_video_id'   => $gig->vimeo_video_id,
            'vimeo_video_link' => $gig->vimeo_video_link
        ]);
    }
    ...
    public function addVimeoVideo()
    {
        // Extract the video ID using preg_match
        $pattern = '/vimeo\.com\/(\d+)/i';
        preg_match($pattern, $this->vimeo_video_link, $matches);
        if (isset($matches[1])) {
            $this->notification([
            'title'       => __('messages.t_success'),
            'description' => __('messages.t_vimeo_video_has_been_successfully_added'),
            'icon'        => 'success'
        ]);

        // Close modal
        $this->dispatchBrowserEvent('close-modal', 'modal-add-vimeo-video-container');
           
        } else {

            // Video not valid, reset form
            $this->reset('vimeo_video_link');

            // Error
            $this->notification([
                'title'       => __('messages.t_error'),
                'description' => __('messages.t_vimeo_video_invalid'),
                'icon'        => 'error'
            ]);
     ...
     * @return void
     */
    public function removeVimeoVideo()
    {
        // Reset vimeo video form
        $this->reset('vimeo_video_link');
    }
    ...
    public function save()
    {
        try {
            ...
            $this->gig->vimeo_video_id        = $this->vimeo_video_link ? vimId($this->vimeo_video_link) : $this->video_id;
            $this->gig->vimeo_video_link      = $this->vimeo_video_link;
